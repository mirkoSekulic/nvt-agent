...
the separately enabled operator-only coordination contract reserves the exact
principal, proves that no matching AgentRun is active, and commits a
target-free authorization. Dynamic admission uses the same durable reservation
so admission and switching cannot overtake each other.

The complete `/v1/principal-accounts/*` API, operator-only
`/v1/principal-account-coordination/*` API, storage algorithm, authentication,
and idempotency windows are documented in
