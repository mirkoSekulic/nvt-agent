...
session host/path, and up to 64 named host-only application exposures. Each
route also carries one bounded internal DNS target and TCP port selected by the
trusted controller from the deterministic run stack. It never carries a URL,
scheme, credential, bearer, provider value, or browser-selectable proxy policy.
