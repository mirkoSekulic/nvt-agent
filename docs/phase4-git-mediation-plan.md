...
- Grant schema for "this is a git grant" — split by trust domain. Broker-side
  ...
  base-url.
- Compose `agent-init`: local compose now mirrors the k8s durable-CA model.
  It creates or reuses `.agents/<name>/egress-ca/ca.crt` and `ca.key`,
  mounts only `ca.crt` into the agent, keeps the key path out of the agent env,
  mounts the keypair only into egressd, and renders `ca.cert_file`/`ca.key_file`
  so an egressd-only restart preserves the CA fingerprint. The CA is initialized
  through the egressd image with the same leaf names egressd validates on load.
  Deleting `.agents/<name>/egress-ca/` rotates the CA on the next mediated
  creation.
