...
Version: v3.8 (Phase 6.2 PR landed: TLS-terminating forward-proxy mode behind `spec.egressForwardProxy` (requires enforcement) — egressd terminates CONNECT under the per-agent CA, injects the broker credential, and re-originates to the pinned upstream, so unmodified tools that honor `HTTP(S)_PROXY` are mediated with zero per-tool config. The per-agent CA leaf-minting is widened from local names only to exactly `localhost` + local Service names + the allowlisted upstream `injection-hosts`, gated by two independent fail-closed layers (CONNECT-host inject-route match + `GetCertificate` SNI allowlist) and the CA's critical name constraints; a leaked key still cannot sign an arbitrary host. `NO_PROXY` is operator-rendered. Codex proof is fixture-level automated (real-Codex go/no-go, gated on refresh mediation, remains manual/optional). Phase 6.1 landed: generic `placeholder-file` materialization.
