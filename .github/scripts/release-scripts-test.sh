...
if NVT_RELEASE_IMAGE_FILTER=nvt-removed-image \
  bash "${ROOT}/.github/scripts/release-images.sh" mirkoSekulic "${FAKE_VERSION}" "${SHA}" "${FAKE_SOURCE}" \
  >/dev/null 2>"${WORKDIR}/removed-filter.err"; then
  echo "unknown release image was accepted" >&2
  exit 1
fi
grep -q 'unknown coordinated release image' "${WORKDIR}/removed-filter.err"
