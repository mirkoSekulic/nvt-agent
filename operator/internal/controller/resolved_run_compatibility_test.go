...
func TestLocalResolvedRunContractMatchesExistingKubernetesProfileResolution(t *testing.T) {
  ...
  "plugins":[
    {"name":"git-host-credentials","source":"builtin","config":{"providers":[{"name":"source","type":"broker","broker-provider":"source-app","match":["github.com/Altinn/altinn-studio"]}]}},
  ...
  "expose":{"http":[{"name":"app","targetPort":3000}]}
	...
	profileSpec.Runtime = nvtv1alpha1.AgentRunRuntime{
		Type: "generic-agent", Autonomy: "trusted-local", User: nvtv1alpha1.AgentRunUserRoot,
	...
	configuration := resolvedrun.TrustedConfiguration{
		Defaults: resolvedrun.PlatformDefaults{
			...
			AgentConfig: append(json.RawMessage(nil), kubernetesRun.Spec.Agent.Config.Raw...),
	...
	if localRun.Image != kubernetesRun.Spec.Image || localRun.Runtime.Type != kubernetesRun.Spec.Runtime.Type ||
		...
		!bytes.Equal(localRun.AgentConfig, kubernetesRun.Spec.Agent.Config.Raw) ||
