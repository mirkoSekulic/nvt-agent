...
func validCodex(access, refresh string) []byte {
	...
	return []byte(fmt.Sprintf(`{"tokens":{"access_token":%q,"refresh_token":%q},"last_refresh":"kept"}`, access+"."+payload+".signature", refresh))
}

func validClaude(access, refresh string) []byte {
	return []byte(fmt.Sprintf(`{"claudeAiOauth":{"accessToken":%q,"refreshToken":%q,"expiresAt":4102444800000}}`, access, refresh))
}
...
func TestCredentialAdaptersAcceptOnlyTheirMinimumRealShape(t *testing.T) {
	...
	invalid := []struct {
		...
		{"duplicate", AdapterClaudeOAuthFile, []byte(`{"claudeAiOauth":{"accessToken":"a","accessToken":"b","refreshToken":"r"}}`)},
