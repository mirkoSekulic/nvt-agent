...
The dashboard defaults to the `Active` view: authorized AgentRuns in Running
phase that have a Ready, Running agent Pod and are therefore routable. The
`All` view also shows authorized Pending, Failed, and otherwise unroutable runs
for diagnosis. Authorization is applied before either view is rendered. These
...
and returns to the mounted gateway root. Logout does not perform provider- or
IdP-specific sign-out.
