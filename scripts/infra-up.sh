...
if [ -f "$broker_dir/local-controller.json" ] && [ -z "${NVT_LOCAL_CONTROLLER_SCHEDULING_CONFIG+x}" ]; then
  export NVT_LOCAL_CONTROLLER_SCHEDULING_CONFIG=/broker-state/local-controller.json
fi
